...
| 17 | Application, Persistence & UI | **Part 1 complete (699 tests)** - `WorldStateStore` persists the whole `WorldState` + `GameCalendar` as one JSON document (reusing `SqliteRepository<T>` unmodified; caught and fixed a real `System.Text.Json` bug along the way - ten get-only `WorldState` dictionaries were silently coming back empty on reload); `WorldSeeder.AssembleWorldState` promoted from a private test helper; `CricketManager.App` - a new fourth project with a headless `new` / `advance` / `status` game loop, testable `AppCommands` behind a thin CLI shim. Build hygiene (`Directory.Build.props`, `.editorconfig`) done. The project is now tracked in git (`git init` + a baseline commit - the owner reversed the earlier "no git" decision mid-pass). **Still open**: a graphical UI, and the live seeded-world domestic-pyramid determinism fix. See `CLAUDE.md` → "PHASE 17 - APPLICATION, PERSISTENCE & UI (PART 1)". |
